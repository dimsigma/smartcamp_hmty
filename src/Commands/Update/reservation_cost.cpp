...
ErrorCode Update::reservationCost(const int argc, const char* const argv[])
{
    ...
    if((!strcmp(argv[1], "--ruid")) && argc == 3){
        sql = (char *) malloc((strlen(this->ReservationCost) * sizeof(char)) + (strlen(argv[1]) * sizeof(char)) + 1);
...
ErrorCode Update::reservationCostShow(const int argc, const char* const argv[])
{
    ...
    if((!strcmp(argv[1], "--ruid")) && argc == 3){
        sql = (char *) malloc((strlen(this->ShowReservationCostInfo) * sizeof(char)) + (strlen(argv[1]) * sizeof(char)) + 1);
