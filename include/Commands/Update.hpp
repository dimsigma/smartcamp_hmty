...
namespace Commands {
    ...
    private:
        ...
        const char* const CustomerMiddleName = "UPDATE CUSTOMER SET middle_name = '%s' WHERE cuid = %s;";
        const char* const CustomerLastName = "UPDATE CUSTOMER SET last_name = '%s' WHERE cuid = %s;";
        ...
        const char* const EditCostService_add = "UPDATE RESERVATION AS res SET res.total_cost = res.total_cost + (SELECT ser.cost FROM SERVICE AS ser WHERE ser.name = '%s') WHERE res.ruid = %s;";
        const char* const ReservationServices_delete = "DELETE FROM INCLUDED_SERVICE WHERE service_name = '%s' AND ruid = %s;";
        const char* const EditCostService_sub = "UPDATE RESERVATION AS res SET res.total_cost  = res.total_cost - (SELECT ser.cost FROM SERVICE AS ser WHERE ser.name = '%s') WHERE res.ruid = %s;";
