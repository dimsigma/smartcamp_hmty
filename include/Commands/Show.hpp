...
namespace Commands {
    ...
    private:
        ...
        const char* const showReservationsSQL = "SELECT RESERVATION.ruid, RESERVATION.total_cost, RESERVATION.checkin_date, RESERVATION.checkout_date, CUSTOMER.* FROM RESERVATION JOIN CUSTOMER ON RESERVATION.main_cuid = CUSTOMER.cuid WHERE %s ORDER BY RESERVATION.checkin_date DESC;";
        ...
        const char* const showCustomersWithoutTotalReservationsSQL = "SELECT CUSTOMER.* FROM CUSTOMER JOIN RESERVATION ON CUSTOMER.cuid = RESERVATION.main_cuid WHERE %s GROUP BY RESERVATION.main_cuid;";
        const char* const showPaymentsSQL = "SELECT PAYMENT.*, CUSTOMER.first_name, CUSTOMER.middle_name, CUSTOMER.last_name, CUSTOMER.cuid FROM PAYMENT JOIN RESERVATION ON PAYMENT.ruid = RESERVATION.ruid JOIN CUSTOMER ON RESERVATION.main_cuid = CUSTOMER.cuid WHERE %s ORDER BY PAYMENT.date DESC;";
        ...
        const char* const showAllAccommodationSQL = "SELECT ACCOMODATION_SPOT.*, SPOT_CLASS.cost FROM ACCOMODATION_SPOT JOIN SPOT_CLASS ON ACCOMODATION_SPOT.class_name = SPOT_CLASS.class_name AND ACCOMODATION_SPOT.class_type = SPOT_CLASS.class_type AND ((SPOT_CLASS.start_date <= CONCAT('1970-', DATE_FORMAT(CURDATE(), '%m-%%d')) AND SPOT_CLASS.end_date >= CONCAT('1970-', DATE_FORMAT(CURDATE(), '%m-%%d'))) OR (SPOT_CLASS.start_date > SPOT_CLASS.end_date AND (CONCAT('1970-', DATE_FORMAT(CURDATE(), '%m-%%d')) >= SPOT_CLASS.start_date OR CONCAT('1970-', DATE_FORMAT(CURDATE(), '%m-%%d')) <= SPOT_CLASS.end_date))) %s WHERE %s ORDER BY ACCOMODATION_SPOT.suid";
        const char* const showAccommodationSQL = "SELECT ACCOMODATION_SPOT.*, %s RESERVATION.*, CUSTOMER.first_name, CUSTOMER.middle_name, CUSTOMER.last_name FROM RESERVATION JOIN CUSTOMER ON RESERVATION.main_cuid = CUSTOMER.cuid NATURAL JOIN RESERVED_SPOT NATURAL JOIN ACCOMODATION_SPOT JOIN SPOT_CLASS ON ACCOMODATION_SPOT.class_name = SPOT_CLASS.class_name AND ACCOMODATION_SPOT.class_type = SPOT_CLASS.class_type AND ((SPOT_CLASS.start_date <= CONCAT('1970-', DATE_FORMAT(RESERVATION.checkin_date, '%m-%%d')) AND SPOT_CLASS.end_date >= CONCAT('1970-', DATE_FORMAT(RESERVATION.checkin_date, '%m-%%d'))) OR (SPOT_CLASS.start_date > SPOT_CLASS.end_date AND (CONCAT('1970-', DATE_FORMAT(RESERVATION.checkin_date, '%m-%%d')) >= SPOT_CLASS.start_date OR CONCAT('1970-', DATE_FORMAT(RESERVATION.checkin_date, '%m-%%d')) <= SPOT_CLASS.end_date))) %s WHERE %s ORDER BY RESERVATION.checkin_date ASC;";
