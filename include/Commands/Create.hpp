...
namespace Commands {
    ...
    private:
        ...
        const char* const serviceCreation = "INSERT INTO SERVICE(name, cost) VALUES('%s', %s);"; // create service --name <name> --cost <cost>

        ErrorCode reservation(const int argc, const char* const argv[]);
        ErrorCode payment(const int argc, const char* const argv[]);
        ErrorCode service(const int argc, const char* const argv[]);
    public:
